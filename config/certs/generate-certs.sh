...
# generate ca
openssl req -x509 -nodes -new -sha256 -days 1024 -newkey rsa:2048 -keyout vault-root-ca.key -out vault-root-ca.pem -subj "/CN=vault.local"
openssl x509 -outform pem -in vault-root-ca.pem -out vault-root-ca.crt

openssl req -new -nodes -newkey rsa:2048 -keyout vault.key -out vault.csr -subj "/CN=vault.local"
